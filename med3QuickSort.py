...
def median(arr, high, low):
    pivot = max(arr[high], arr[low], arr[(high + low) // 2])
    for i in (high, low, (high + low) // 2):
        if pivot == arr[i]:
            pivPos = i
    return pivot, pivPos

def organize(arr, low, high):
    pivot, pivPos = median(arr, high, low)
    arr[pivPos], arr[high], pivPos = arr[high], arr[pivPos], high
    i = low - 1
    for j in range(low, pivPos):
        if arr[j] <= pivot:
            i += 1
            arr[i], arr[j] = arr[j], arr[i]
        drawArray(arr, i, j, pivot)
    arr[i + 1], arr[pivPos] = arr[pivPos], arr[i + 1]
    return i + 1
